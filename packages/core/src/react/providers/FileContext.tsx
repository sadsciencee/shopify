...
import {
    ...
    type MimeType, filenameFromPreviewUrl, UploadedFile
...
type FileContextType = {
    ...
    getFileById: (
        id: string,
    ) => UploadedFile | SearchFile | LocalFile | undefined;
...
export const FileProvider = ({
    ...
    const {executeQuery, loading, value: searchValue, resetQueryCache} = useDirectAccess<{
        files: { nodes: FileSearchNode[] };
    }>();

    const [selectionCallbacks] = useState(
        () => new Set<FileSelectionCallback>(),
    );
    ...
    const getUploadingFiles = useCallback(() => {
        return Object.values(state.localFiles)
            .filter(
                (file): file is typeof file & { isUploading: true } =>
                    file.isUploading,
            )
            .map((file) => ({
                id: file.id,
        ...
        (file: FileSearchNode) => {
            ...
            importFetcher.submit(
                JSON.stringify({
                    id: cleanId,
                    filename:
                        filenameFromPreviewUrl(file)?.filename ?? "New File",
                }),
                {
                    method: "POST",
        ...
        <FileContext.Provider
            value={{
                ...
                state: {
                    ...
                    hasUploadingFiles: Object.values(state.localFiles).some(
                        (f) => f.isUploading,
                    ),
